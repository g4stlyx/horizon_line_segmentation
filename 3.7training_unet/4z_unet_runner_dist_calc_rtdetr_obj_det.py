"""
================================================================================
U-Net Horizon Segmentation + Object Distance Calculator (Using HF RT-DETR)
================================================================================

Adds distance computation between per-object centers (ships, buoys etc.) and the
estimated horizon line derived from a sky/non-sky U-Net segmentation mask.

Key Arguments:
    --band-up / --band-down : vertical search band around horizon (pixels)
    --min-area              : minimum blob area to consider an object
    --fov-vertical          : adds vertical angle column (deg)
    --frame-step            : process every N-th frame for video
    --no-ship-correction    : disable heuristic ship correction on mask
...
def create_overlay(image_bgr, mask):
    ...
    overlay[mask == 1] = (255, 0, 0)  # Blue-ish (B,G,R)
...
def compute_horizon_line(mask):
    """Return (raw_horizon, smoothed_horizon) arrays length W with bottom-most sky pixel per column."""
    h, w = mask.shape
    sky = (mask == 1)
    horizon = np.full(w, h//2, dtype=np.int32)
    # per-column scan
    for col in range(w):
        col_sky = sky[:, col]
        if col_sky.any():
            horizon[col] = np.max(np.where(col_sky)[0])
    # moving average smoothing
    k = 21
    pad = k // 2
    smoothed = horizon.astype(np.float32).copy()
    for i in range(w):
        l = max(0, i - pad)
        r = min(w, i + pad + 1)
        smoothed[i] = np.mean(horizon[l:r])
    return horizon, smoothed.astype(np.int32)
...
def detect_objects_near_horizon(
    ...
    """Detect non-sky blobs within a vertical band around the horizon and
    compute their vertical distance to the horizon.

    This replaces the previous "above horizon only" logic so that ships below the
    horizon are also detected. The measurement point is chosen as the bbox edge
    (top or bottom) that is closest to the horizon.
    """
    h, w = mask.shape
    ...
    # Objects are non-sky within the band
    obj_mask = ((mask == 0) & band_mask).astype(np.uint8)
    if obj_mask.sum() == 0:
        return []
...
def annotate(image_bgr, mask, horizon_smoothed, objects, show_horizon=True, fps=None):
    ...
    for idx, obj in enumerate(objects):
        ...
        if obj.get('det') == 'rtdetr':
            ...
            label = f"ID{idx} {class_name} {conf:.2f} d={dist}px"
        else:
            box_color = (0, 255, 0)  # Green for segmentation detections
            label = f"ID{idx} d={dist}px"
